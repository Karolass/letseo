...
module.exports = function (options) {
  ...
  if (options.file) {
    if (checkFileExist(options.file.input)) {
      const selector = new Selector(rules)
      const printer = new Printer(options.print == undefined || options.print)

      fs.createReadStream(options.file.input)
        .pipe(selector)
        .pipe(printer)
        .pipe(fs.createWriteStream(options.file.output))
    }
  }
}
